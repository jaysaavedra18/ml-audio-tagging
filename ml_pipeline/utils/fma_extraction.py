...
class FreeMusicArchive:
    ...
    def _get_data(self, dataset, fma_id, fields=None):
        ...
        if r_id != str(fma_id):
            raise Exception(
                f"The received id {r_id} does not correspond to"
                f"the requested one {fma_id}",
            )
        if fields is None:
            return data
    ...
    def get_all_genres(self):
        ...
        while True:
            ...
            url += f"&page={page}&api_key={self.api_key}"
            r = requests.get(url)
    ...
    def find_roots(self):
        ...
        for gid, row in self.df.iterrows():
            ...
            if parent == 0:
                ...
                msg = f"{gid} ({title}) has parent {parent} which is missing"
...
def build_sample_loader(audio_dir, Y, loader):
    class SampleLoader:
        def __init__(self, tids, batch_size=4):
            ...
            self.Y = np.empty((self.batch_size, Y.shape[1]), dtype=int)
